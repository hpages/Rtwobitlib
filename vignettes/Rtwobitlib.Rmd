...
date: "Last edited 17 April 2024"
package: Rtwobitlib
vignette: >
...
**Rtwobitlib** is an R package that provides the 2bit C Library
from the UCSC Genome Browser.
See [this page](https://genome.ucsc.edu/FAQ/FAQformat.html#format7)
on the UCSC Genome Browser website for a quick overview of the 2bit format.

The **Rtwobitlib** package is primarily useful to developers
of other R packages who wish to use the 2bit C library in their own C code.
...
In order for the C/C++ compiler to find the 2bit header files during
...
specifications, e.g., `LinkingTo: Rtwobitlib (>= 0.3.0)`.
...
To compile and link your package successfully against the 2bit library
...
`Rtwobitlib` provides both static and dynamic versions of the 2bit
library on Linux, but only the static version on Windows and macOS.
The procedure above will link against the static version of the 2bit
...
These functions are implemented in C on top of the 2bit library bundled
in the package.
